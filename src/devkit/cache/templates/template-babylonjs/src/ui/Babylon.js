...
exports = Class(function(supr) {
  ...
  this.onResize = function(ev) {
    if (this.canvas == void 0) {
      this.canvas = window.__canvas = window.__canvas || document.getElementById(this.id);
    }

    this.canvas.width = ev.target.innerWidth;
    this.canvas.height = ev.target.innerHeight;

    device.screen.width = this.canvas.width;
    device.screen.height = this.canvas.height;
    this.engine.resize();

    // propagate this into current active scenes
  ...
  // -------------------------------------------------------------------------------------
  this.renderSchema = function(_parentView, schema, multiplier, defaultPlaneSize, camera, scene, __) {
    /* TOOLS integrations for Schema Editor*/
    var root;
    var schema = jsio('import src.ui.schemas.' + schema);
    var self = _parentView; // ugh!
    var that = this;
    recursiveTreeDrill = function(treeNode, parent) {
      ...
      if (treeNode['width'] !== void 0) {
        ref.width = (treeNode['width'] * defaultPlaneSize.width) / device.screen.width;
      }
      if (treeNode['height'] !== void 0) {
        ref.height = (treeNode['height'] * defaultPlaneSize.height) / device.screen.height;
      }
      ...
      for (var node in treeNode) {
        ...
        _node.view.width = ref.width * Number(_node.width || 1);
        _node.view.height = ref.height * Number(_node.height || 1);
        ...
        _node.view.material = new BABYLON.StandardMaterial(_node.id);
        if (_node.imageName) {
          ...
          _node.view.updateText = bind(_node, function(ctx, w, h, text, scale, color) {
            ...
            textureContext.font = size + "px " + (this.fontFamily || "Arial");
            var textWidth = Math.max(w, textureContext.measureText(text).width);
            //Calculate ratio of text width to size of font used
            var ratio = textWidth / size; // 12 or any value
            var fontSize = Math.floor(w / (ratio * 1)); //size of multiplier (1) can be adjusted, increase for smaller text
            // fix ratio according to editor
            fontSize *= ((_node.size || 24)) / 24;
            var font = "" + Math.floor(fontSize) + "px " + (this.fontFamily || "Arial");
            ...
            ctx.drawText(text, alignment, null, font, color || this.color, this.backgroundColor, true, true);
            //ctx.update();
          }, dynamicTexture, DTWidth, DTHeight);

          _node.view.updateText(text, (ref.height) * (_node.size / 24));
          ...
          _node.view.material.diffuseColor = new BABYLON.Color3(0, 0, 0);
          _node.view.material.emissiveColor = new BABYLON.Color3(color.r / 255, color.g / 255, color.b / 255);
        ...
        _node.view.mesh.origin = {
          x: _node.view.mesh.position.x || 0,
          y: _node.view.mesh.position.y || 0,
          z: _node.view.mesh.position.z || 0
        }

        _node.view.mesh.resetOrigin = function(newPos) {
          _node.view.mesh.position.x = newPos.x;
          _node.view.mesh.position.y = newPos.y;
          _node.view.mesh.position.z = newPos.z;
          _node.view.mesh.origin = {
            x: newPos.x,
            y: newPos.y,
            z: newPos.z
    ...
    recursiveTreeDrill(schema, __);
    // input handling loop
    var INPUTS_UPDATE = false;
    var activeTextNode;
    if (!_parentView.registeredObservable) {
      _parentView.registeredObservable = true;

      scene.onPointerObservable.add(bind(_parentView, function(pointerInfo) {
  ...
  // -------------------------------------------------------------------------------------
  this.renderSchema2Panel = function(_parentView, panel, schema, camera, scene) {
    /* TOOLS integrations for Schema Editor*/
    var root;
    ...
    recursiveTreeDrill(schema, panel);
    // input handling loop
    var INPUTS_UPDATE = false;
    var activeTextNode;
    if (!_parentView.registeredObservable) {
  ...
  this.showAxis = function(size, scene) {
    var makeTextPlane = function(text, color, size) {
      ...
      plane.material = new BABYLON.StandardMaterial("AxisMat" + text, scene);
