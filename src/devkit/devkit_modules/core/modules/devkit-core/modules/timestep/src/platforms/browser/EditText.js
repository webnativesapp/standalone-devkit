...
exports = Class(function () {
  ...
  this.refresh = function(value, hasBack, hasForward, cursorPos) {
    ...
    var scale = ((timestep_onscreen_canvas.offsetWidth*100)/device.width) / 100;
    ...
    //_input.setHorizontalPadding(0, 0); //this._opts.paddingLeft * scale, this._opts.paddingRight * scale);// OFS hack NO PADDING
    _input.setFontSize(textBox.getOpts().size * scale);
    _input.setFontFamily(textBox.getOpts().fontFamily)
    _input.setPosition(pos.x * scale, pos.y * scale, pos.width * scale , pos.height * scale);
    _input.show();
