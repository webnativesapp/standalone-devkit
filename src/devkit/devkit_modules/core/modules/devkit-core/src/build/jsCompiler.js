...
exports.JSCompiler = Class(function () {
    this.init = function (api, app, config, jsConfig) {
        this.bubleTranspiler = require('buble');
    ...
    this.compile = function (opts, cb) {
        this.errorsLogger = opts.errorsLogger;
    ...
    this.compress = function (filename, src, opts, cb) {
        var errLogger = this.errorsLogger;
        // yuck!
        try {
            cb(null, this.bubleTranspiler.transform(src).code);
        } catch (ex) {
            try {
                this.babel.transform(src, {
        ...
        //   tool: 'closure',
        //   args: closureOpts,
        //   stdin: this.bubleTranspiler.transform(src).code,
        //   buffer: true
...
var DevKitJsioInterface = Class(EventEmitter, function () {
     ...
     */
    var compressCacheLibsMap = {};
    var cacheLibPaths = ['/node_modules', '/devkit_modules', '/modules'];
    var checkForLibPath = function (filename) {
        var res = false;
        for (var i in cacheLibPaths) {
            res = res || filename.split('\\').join('/').includes(cacheLibPaths[i]);
            if (!!res) {
                break;
            }
        }
    ...
    this.compress = function (filename, src, opts, cb) {
        ...
        if (compressCacheLibsMap[filename]) {
            cb && cb(compressCacheLibsMap[filename]);
        } else if (opts.compressorCachePath && filename) {
            try {
        ...
        function onCompress(err, src) {
            ...
            bridge.strip(src, opts, function (err, src) {
                if (filename && checkForLibPath(filename)) {
                    compressCacheLibsMap[filename] = src;
                }
                cb(src);
            });
        }
